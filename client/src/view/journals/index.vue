    ...
    <el-row :gutter="10" @page-change="fetch">
      <el-col :span="12">
        <div>
          <el-button
            round
            @click="flagCreateJournal=true"
            style="float:left;margin: 10px 10px 20px 10px; "
            icon="el-icon-plus"
        ...
        <el-card class="box-card" style="margin-bottom:20px;">
          <div slot="header" class="clearfix" style="text-align:left;margin-left:10px">
            ...
            <div style="float:right">
              ...
              <el-button
                v-show="journal.followed===false"
                v-on:click="handleFollowJournal(key,journal._id)"
                type
                plain
                round
              >Follow the journal</el-button>
              <el-button
                v-show="journal.followed===true"
                v-on:click="handleUnFollowJournal(key,journal._id)"
                type="primary"
                plain
                round
              >Followed</el-button>
          ...
          <div class="body">
            <el-row :gutter="10">
                  ...
                  <g>
                    <defs>
                      <linearGradient :id="journal._id" x1="0%" y1="0%" x2="100%" y2="0%">
                        <stop
                          offset="0%"
                          v-bind:style="{'stop-color': journal.color_1 , 'stop-opacity':1}"
                        />
                        <stop
                          offset="100%"
                          v-bind:style="{'stop-color': journal.color_2 , 'stop-opacity':1}"
                        />
                      </linearGradient>
                    </defs>
                    <rect
                    ...
                    <rect
                      ...
                      width="100"
                      height="130"
                      style="fill:url(#grad1);stroke-width:1;stroke:rgb(51,51,51)"
                    />
                    <text
                      x="20"
                      y="30"
              ...
              <el-col :span="6" :offset="2">
                ...
                {{ journal.creationDate | moment("DD/MM/YYYY") }}
                <p>
                  <strong>Editor-In-Chief:</strong>
                  <a
                    v-for="item in journal.users"
                    href="#"
                    title="author"
        ...
        .then(async list => {
          ...
          for (let i = 0; i < this.journals.length; i++) {
            for (let j = 0; j < this.followedJournals.length; j++) {
              if (
                this.journals[i]._id === this.followedJournals[j].id_journal._id
              ) {
