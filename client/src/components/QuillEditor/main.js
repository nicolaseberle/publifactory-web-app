...
doc.subscribe(function(err) {
	...
	document.addEventListener('cursors-update', function(e) {
		...
		e.detail.removedConnections.forEach(function(connection) {
			if (cursorsModule.cursors[connection.id]) cursorsModule.removeCursor(connection.id);
		});
...
function updateUserList() {
	...
	cursors.connections.forEach(function(connection) {
		...
		userNameEl.innerHTML =
			'<strong>' + (connection.name || '(Waiting for username...)') + '</strong>';
		userNameEl.classList.add('user-name');

		if (connection.id == cursors.localConnection.id) userNameEl.innerHTML += ' (You)';

		if (connection.range) {
			if (connection.id == cursors.localConnection.id) connection.range = quill.getSelection();
		...
		} else userDataEl.innerHTML = '(Not focusing on editor.)';
