...
class Citation extends InlineBlot {
  ...
  update(mutations, context) {
    ...
    // console.log('CITATION_LOG', target.added[0].textContent);
    // if (target.addedNodes[0].textContent.match(/\[([0-9]+)\]/)) {
    // console.log('CITATION_LOG', target.addedNodes[0].textContent);
    // if (target.addedNodes[0].textContent.length > 3) {
    //   target.addedNodes[0].textContent =
    //     target.addedNodes[0].textContent.slice(0, 2) + ']';
    //   // }
    // }
  ...
  mounted() {
    ...
    this.editor.on('text-change', (delta, oldDelta, source) => {
      const citationsDeleted = oldDelta.reduce((acc, op) => {
        if (op.attributes && op.attributes.citation) {
          const element = document.querySelectorAll(
            `a[href^='${op.attributes.citation}']`
          )[0];
          if (!element) {
            return [...acc, op.attributes.citation];
          }
        }
        return acc;
      }, []);
